...
The Direct Hardware adapter lets CitraScope control your telescope devices end-to-end — no intermediary software like N.I.N.A. or KStars is needed. CitraScope communicates directly with your cameras, mounts, focusers, and filter wheels over USB or serial.
...
- **Full CitraScope integration** — autofocus, alignment, pointing model calibration, camera preview, and calibration frame capture are all built in
...
| **USB Camera** (via OpenCV) | `uv tool install citrascope --with citrascope[usb-camera]` | Guide cameras, planetary cameras, webcams |
| **Raspberry Pi HQ Camera** (IMX477) | `uv tool install citrascope --with citrascope[rpi]` | For Pi-based deployments |
...
| Device | Install Extra | Notes |
|--------|--------------|-------|
| **ZWO AM3 / AM5 / AM7** | `uv tool install citrascope --with citrascope[zwo-mount]` | USB serial or WiFi TCP connection |

{: .note }
> You can run CitraScope without a mount for static camera setups. Leave the mount type empty in configuration.
...
1. Open the CitraScope web interface (default: `http://localhost:24872`)
...
4. Click **Save Configuration**. CitraScope will connect to each selected device and report status in the dashboard.
...
**Solutions**:
- Install the required extras: `uv tool install citrascope --with citrascope[zwo-mount]` (or whichever extra)
- Restart CitraScope after installing new packages
- Check that the device is physically connected (USB, serial)

### Connection Fails

**Problem**: CitraScope reports a device as disconnected after saving configuration.
...
- Review the CitraScope log for detailed error messages (Monitoring tab → expand the log panel)
